...
try:
    ...
    # The core UI must still boot when the optional browser agent dependency
    # is not installed in the current environment.
    if exc.name and not exc.name.startswith("browser_use"):
        raise
    ChatGoogle = None  # type: ignore[assignment]
    browser_use_output_sanitize = None  # type: ignore[assignment]
...
def gemini_clean_and_conform(text: str):
    if not _BROWSER_USE_AVAILABLE or browser_use_output_sanitize is None:
        return None
...
def apply():
    ...
    if not _BROWSER_USE_AVAILABLE or ChatGoogle is None:
        return
