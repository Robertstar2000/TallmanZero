...
def parse_state_request_payload(payload: Mapping[str, Any]) -> StateRequestV1:
    ...
    return StateRequestV1(
        ...
        timezone=tz,
        user_id=payload.get("user_id"), # usually passed from wrapper
    )
...
def _coerce_state_request_inputs(
    *,
    ...
    return StateRequestV1(
        ...
        timezone=tz,
        user_id=None, # To be set by caller
    )
...
async def build_snapshot_from_request(*, request: StateRequestV1) -> SnapshotV1:
    ...
    active_context = AgentContext.get(ctxid, user_id=request.user_id) if ctxid else None
    ...
    all_ctxs = AgentContext.all(user_id=request.user_id)
    for ctx in all_ctxs:
...
async def build_snapshot(
    *,
    ...
    # Set user_id explicitly as _coerce_state_request_inputs returns it as None by default
