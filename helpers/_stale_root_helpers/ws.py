...
class WsHandler:
    ...
    def use_context(self, ctxid: str, create_if_not_exists: bool = True):
        ...
        user_id = None
        # get sid from current context if possible, though WsHandler should usually have it
        # for now, we try to get it from the manager's tracking if available
        # but the safest is to pass it from the handler if we know which SID we are processing.
        # WsHandler.process(..., sid) passes the sid.
        # If we are inside process(), we don't have an easy way to get the sid here 
        # unless we store it in a thread-local or similar.
        
        # Better: WsHandler should be used with sid knowledge.
        # I'll add a 'sid' attribute to WsHandler during process().
        sid = getattr(self, "_current_sid", None)
        if sid:
            with ws._contexts_lock:
                sctx = ws._ws_contexts.get(sid)
                if sctx:
                    user_id = sctx.user_id

        return _use_context(self.lock, ctxid, create_if_not_exists, user_id=user_id)
...
def _check_security(handler_cls: type[WsHandler], ctx: _SecurityContext) -> dict[str, Any] | None:
    ...
    if handler_cls.requires_auth():
        ...
        if login.is_login_required() and not ctx.user:
        ...
        try:
            ...
            for path, instance in activated.items():
                if instance not in passing_handlers:
                    continue
                instance._current_sid = sid # type: ignore
                try:
