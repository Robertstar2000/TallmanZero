...
class AgentContext:
    ...
    def __init__(
        ...
        if self.user_id is not None:
             ...
             existing_mapping = database.get("SELECT * FROM user_contexts WHERE ctxid = ?", [self.id])
             if not existing_mapping:
                 database.run("INSERT INTO user_contexts (ctxid, user_id) VALUES (?, ?)", [self.id, self.user_id])
