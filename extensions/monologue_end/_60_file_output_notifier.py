...
class FileOutputNotifier(Extension):
    ...
    async def execute(self, **kwargs):
        ...
        PrintStyle.debug(f"[FileOutputNotifier] Checking for output files in context {self.agent.context.id}")
        ...
        try:
            ...
            all_text = ""
            bash_text = ""
            for item in items[-20:]: # focus on recent activity
                c = (item.content or "")
                h = (item.heading or "")
                all_text += " " + h + " " + c
            ...
            if downloadable:
                PrintStyle.info(f"[FileOutputNotifier] Detected {len(downloadable)} output file(s).")
                for f in downloadable:
                    PrintStyle.info(f"  - {f['name']} ({f['path']})")
                ...
                PrintStyle.debug("[FileOutputNotifier] No new files detected.")
