...
function afterMessagesUpdate(logs) {
  ...
  // TallmanZero: dispatch events for file_output log items so the download modal appears
  // Files are stored in kvps.files (list) - content is just a human-readable summary string
  for (const log of logs) {
    if (log.type === "file_output") {
      const files = log.kvps && Array.isArray(log.kvps.files) ? log.kvps.files : [];
      if (files.length > 0) {
        console.log("[TallmanZero] file_output detected:", files);
        document.dispatchEvent(new CustomEvent("a0_log_event", { detail: { ...log, files } }));
      }
    }
  }
